...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # Self-computed ELO covers all 321 teams (the bundled file only had 48 WC
    # qualifiers, causing PoissonModel to silently drop ~87% of training matches).
    # Cache is generated by `uv run wc2026 refresh-data` (or any cli command).
    cache_path = DATA_DIR.parent / "elo_history_computed.csv"
    if not cache_path.exists():
        raise FileNotFoundError(
            f"Missing {cache_path}. Run `uv run wc2026 refresh-data` to generate it."
        )
