...
| `poisson-loser-margin` | Loser's goals and the winner-loser margin each fit directly as `PoissonRegressor` targets (elo z-gap, mean elo z, neutral); margin (0 = draw, m >= 1 = favorite wins by m) is chosen by `EV = 2*P(exact) + P(outcome)` using `poisson+elo`'s outcome probabilities. |
